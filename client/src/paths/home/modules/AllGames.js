...
function AllGames(){
    const [params, setParams] = useState(<Redirect to={`/?month=${formattedMonthYear.getMonth()}&year=${formattedMonthYear.getFullYear()}`} />)
    ...
    const handleMonth = (num) => {
        ...
        // console.log(formattedMonthYear)
        return setParams(<Redirect to={`/?month=${formattedMonthYear.getMonth()}&year=${formattedMonthYear.getFullYear()}`} />)
    }
    useEffect(() => {
        return setParams
    }, [setParams])
